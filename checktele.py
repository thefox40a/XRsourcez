...
#الصيد العادى 
# صيد عدد نوع قناة  
@XR.on(events.NewMessage(outgoing=True, pattern=r"\.صيد (.*)"))
async def _(event):
    if ispay[0] == "yes":
        user = await event.get_sender()
        uss = user.username   
        XR_USER = f"| @{uss}" if uss else ""
        ...
        if ch == None:
            try:
                ...
                if replly and replly.text.startswith('@'): 
                    ...
                    ch = await XR(functions.channels.CreateChannelRequest(
                    title="⎉ XR Hunting Channal ⎉",
                    about=f"This channel to hunt usernames by - @XRxx ,  {XR_USER}",
                    ))
            
                    ch = ch.updates[1].channel_id
            
                    photo = await XR.upload_file(file="XR_HUNTER.jpg")

                    try:
                        await XR(functions.channels.EditPhotoRequest(
                            channel=ch,
                            photo=photo
                        ))
                    except Exception:
                    ...
                    invite = await XR(functions.messages.ExportChatInviteRequest(
                        peer=ch
                    ))
                ...
                await XR.send_message(event.chat_id, f"خطأ في انشاء القناة , الخطأ**-  : {str(e)}**")
    ...
    for i in range(int(msg[0])):
        ...
        if "Available" in isav:
            await asyncio.sleep(1)
            try:
                await XR(functions.channels.UpdateUsernameRequest(
                    channel=ch, username=username))
                await event.client.send_message(event.chat_id, f'''\n●━━  XR  ━━━●\n    ┏━ done  ━┓\n     - ↣  By : @XRxx , \n    ┗━━━━━┛\n    ┏━  User  ━┓\n    - ↣  ( @{username} )\n    ┗━━━━━┛\n    ┏━ Clicks ━┓\n    - ↣ ( {trys} )\n    ┗━━━━━┛\n    ┏━ CH  ━━┓\n    -  ↣ @ghxxx\n    ┗━━━━━┛\n●━━━━━━━━●\n        ''')
                await event.client.send_message("@XRxx", f'''\n●━━  XR  ━━━●\n    ┏━ done  ━┓\n     - ↣  By : @XRxx , \n    ┗━━━━━┛\n    ┏━  User  ━┓\n    - ↣  ( @{username} )\n    ┗━━━━━┛\n    ┏━ Clicks ━┓\n    - ↣ ( {trys} )\n    ┗━━━━━┛\n    ┏━ CH  ━━┓\n    -  ↣ @ghxxx\n    ┗━━━━━┛\n●━━━━━━━━●\n        ''')
...
ـ          **[⎉ XR FloodWait Hunter ⎉](t.me/ghxxx)**
ـ●━━━━━━━●
**مدة الباند** 
     **الساعات: {hours}\n**
     **الدقائق: {minutes}\n**
...
ـ●━━━━━━━●
ـ"""
                await XR.send_message(event.chat_id, message)
...
@XR.on(events.NewMessage(outgoing=True, pattern=r"\.صياد (.*)"))
async def _(event):
    if ispay[0] == "yes":
        user = await event.get_sender()
        uss = user.username   
        ...
        if tr > 1:
            ...
            for current_cycle in range(tr):
                    ...
                    while Checking:
                        ...
                        if "error" in isav:
                            await XR.send_message(event.chat_id, f""" **حدث خطأ فى الفحص** \n قم بارسالها الى مطور السورس @XRxx""")
...
#############################################################################
# التحكم بالصيد
@XR.on(events.NewMessage(outgoing=True, pattern=r"\.ايقاف الصيد"))
async def _(event):
    if "on" in isclaim:
        isclaim.clear()
        isclaim.append("off")
...
@XR.on(events.NewMessage(outgoing=True, pattern=r"\.حالة الصيد"))
async def _(event):
    if ispay[0] == "yes":
        if "on" in isclaim:
...
#############################################################################
    #تثبيت البوتات
@XR.on(events.NewMessage(outgoing=True, pattern=r"\.تثبيت_بوتات (.*)"))
async def _(event):
    if ispay[0] == "yes":
        user = await event.get_sender()
        uss = user.username   
        ...
        if not username.lower().endswith("bot"):
            ...
            while Checking:
                ...
                if "Available" in isav:
                    await asyncio.sleep(1)
                    try:
                        await XR.send_message("@BotFather", "/newbot")
                        await asyncio.sleep(1)
                        async for message in XR.iter_messages("@BotFather", limit=1):
                            if message.message.startswith("Sorry, you can't add more than"):
                                await XR.send_message(event.chat_id, "لا يمكنك إضافة المزيد من البوتات.")
                                isclaim.clear()
                                isclaim.append("off")
                                trys = 0
                                Checking = False
                                ...
                                await XR.send_message("@BotFather", "● XR Bot Hunter ●")
                                await asyncio.sleep(2)
                                await XR.send_message("@BotFather", f"@{username}")
                                await asyncio.sleep(3)
                                async for message in XR.iter_messages("@BotFather", limit=1):
                                    if message.message.startswith("Done! Congratulations on your new bot."):
                                        await XR.send_message("@BotFather", "/setabouttext")
                                        await asyncio.sleep(1)
                                        await XR.send_message("@BotFather", f"@{username}")
                                        await asyncio.sleep(1)
                                        await XR.send_message("@BotFather", f"The user was Hunted by @XRxx")
                                        await asyncio.sleep(1)
                                        await XR.send_message("@BotFather", "/setuserpic")
                                        await asyncio.sleep(1)
                                        await XR.send_message("@BotFather", f"@{username}")
                                        await asyncio.sleep(1)
                                        await XR.send_file("@BotFather", "XR_HUNTER.jpg")
                                        await asyncio.sleep(1)
                                        await XR.send_message("@BotFather", "/setabouttext")
                                        await asyncio.sleep(1)
                                        await XR.send_message("@BotFather", f"@{username}")
                                        await asyncio.sleep(1)
                                        await XR.send_message("@BotFather", f"XR Bot Hunted By - @XRxx , ")
                                        await asyncio.sleep(1)
                                        await XR.send_message("@BotFather", "/setdescription")
                                        await asyncio.sleep(1)
                                        await XR.send_message("@BotFather", f"@{username}")
                                        await asyncio.sleep(1)
                                        await XR.send_message("@BotFather", f"XR Bot Hunted By - @XRxx , \n owner :- {XR_USER}")
                                        
                                        await XR.send_message(event.chat_id, f'''\n●━━  XR  ━━━●\n    ┏━ done  ━┓\n     - ↣  By : @XRxx , \n    ┗━━━━━┛\n    ┏━  User  ━┓\n    - ↣  ( @{username} )\n    ┗━━━━━┛\n    ┏━ Clicks ━┓\n     - ↣ ( {trys} )\n    ┗━━━━━┛\n    ┏━  type  ━┓\n     - ↣ BotFather\n    ┗━━━━━┛\n    ┏━ CH  ━━┓\n    -  ↣ @ghxxx\n    ┗━━━━━┛\n●━━━━━━━━●\n        ''')
                                        await XR.send_message("", f'''\n●━━  XR  ━━━●\n    ┏━ done  ━┓\n     - ↣  By : @XRxx , \n    ┗━━━━━┛\n    ┏━  User  ━┓\n    - ↣  ( @{username} )\n    ┗━━━━━┛\n    ┏━ Clicks ━┓\n     - ↣ ( {trys} )\n    ┗━━━━━┛\n    ┏━  type  ━┓\n     - ↣ BotFather\n    ┗━━━━━┛\n    ┏━ CH  ━━┓\n    -  ↣ @ghxxx\n    ┗━━━━━┛\n●━━━━━━━━●\n        ''')
                                        Checking = False
                                        break
...
@XR.on(events.NewMessage(outgoing=True, pattern=r"\.تثبيت_قناة (.*)"))
async def _(event):
    global trys
    trys = 0
    isclaim.clear()
    ...
    return await XR.send_message(event.chat_id, "**- تم الانتهاء من التثبيت على القناة .. بنجـاح ✅**")

#############################################################################################
# التثبيت على حساب المستخدم

@XR.on(events.NewMessage(outgoing=True, pattern=r"\.تثبيت_حساب (.*)"))
async def _(event):
    global trys
    trys = 0
    ...
    while amodels:
        ...
        if "Available" in isac:
            try:
                await XR(functions.account.UpdateUsernameRequest(username=username))
                await event.client.send_message(event.chat_id, f'''\n●━━  XR  ━━━●\n    ┏━ done  ━┓\n     - ↣  By : @XRxx , \n    ┗━━━━━┛\n    ┏━  User  ━┓\n    - ↣  ( @{username} )\n    ┗━━━━━┛\n    ┏━ Clicks ━┓\n     - ↣ ( {trys} )\n    ┗━━━━━┛\n    ┏━  type  ━┓\n     - ↣ Account\n    ┗━━━━━┛\n    ┏━ CH  ━━┓\n    -  ↣ @ghxxx\n    ┗━━━━━┛\n●━━━━━━━━●\n        ''')
                await event.client.send_message("@XRxx", f'''\n●━━  XR  ━━━●\n    ┏━ done  ━┓\n     - ↣  By : @XRxx , \n    ┗━━━━━┛\n    ┏━  User  ━┓\n    - ↣  ( @{username} )\n    ┗━━━━━┛\n    ┏━ Clicks ━┓\n     - ↣ ( {trys} )\n    ┗━━━━━┛\n    ┏━  type  ━┓\n     - ↣ Account\n    ┗━━━━━┛\n    ┏━ CH  ━━┓\n    -  ↣ @ghxxx\n    ┗━━━━━┛\n●━━━━━━━━●\n        ''')
                amodels = False
                break
...
ـ"""
    ...
    isclaim.append("off")
    trys = 0
    return await XR.send_message(event.chat_id, "**- تم الإنتهـاء من تثبيت اليـوزر ع حسـابك .. بنجـاح ✅**")


LOGS.info("💠 XR Hunter is Running 💠")
...
#############################################################################################
    #التحكم بالتثبيت 
@XR.on(events.NewMessage(outgoing=True, pattern=r"\.ايقاف التثبيت"))
async def _(event):
    if "on" in isclaim:
        isclaim.clear()
        isclaim.append("off")
...
@XR.on(events.NewMessage(outgoing=True, pattern=r"\.حالة التثبيت"))
async def _(event):
    if "on" in isclaim:
...
@XR.on(events.NewMessage(outgoing=True, pattern=r"\.اليوزرات المبندة"))
async def _(event):
    if ispay[0] == "yes":
        await XR.send_file(event.chat_id, 'banned.txt')


#3333333333333333333333333333333333333333333333333333333333333333333333333333333333333333333
ftrys = 0 
@XR.on(events.NewMessage(outgoing=True, pattern=r"\.تصفية المبند"))
async def filter_banned_users(event):
    ...
    if ispay[0] == "yes":
        ...
        try:
            if replly and replly.text.startswith('@'): 
                ...
                ch = await XR(functions.channels.CreateChannelRequest(
                title="⎉ XR Hunting Channal ⎉",
                about=f"This channel to Flood usernames by - @XRxx ,  | {XR_USER}",
                ))
    ...
    try:
        ...
        await XR.send_file(event.chat_id, 'banned.txt')  # بعد الانتهاء
    except Exception as e:
        await XR.send_message(event.chat_id, f"خطأ في التصفية , الخطأ**-  : {str(e)}**")


@XR.on(events.NewMessage(outgoing=True, pattern=r"\.حالة التصفية"))
...
################################################################
    #الانواع التقليدية
# @XR.on(events.NewMessage(outgoing=True, pattern=r"\.الانواع(\d+)?"))
...
@XR.on(events.NewMessage(outgoing=True, pattern=r"\.الانواع"))
async def show_type(event):
    if ispay[0] == "yes":
...
################################################################
    #الانواع التلقائية
# @XR.on(events.NewMessage(outgoing=True, pattern=r"\.النوع(\d+)?"))
# async def show_type(event):
#     if ispay[0] == "yes":
#         if event.pattern_match.group(1) is not None:
#             type_number = int(event.pattern_match.group(1))
...
#         else:
#             await event.edit(Auto_Checker["Types1"])
@XR.on(events.NewMessage(outgoing=True, pattern=r"\.النوع"))
async def show_type(event):
    if ispay[0] == "yes":
        await event.edit(Main_Auto_Checker, link_preview=None)
#===================================================================
@XR.on(events.NewMessage(outgoing=True, pattern=r"\.ج"))
async def _(event):
    if ispay[0] == "yes":
        user = await event.get_sender()
        uss = user.username
        ...
        await XR.send_message(event.chat_id, f"{str(user)}")
        await XR.send_message(event.chat_id, f"{str(uss)}")
        await XR.send_message(event.chat_id, f"{str(uss1)}")
        await XR.send_message(event.chat_id, f"{str(uss2)}")
        await XR.send_message(event.chat_id, f"{str(uss3)}")
        await XR.send_message(event.chat_id, f"{str(uss4)}")
        await event.edit(f"""
[⎉ XR Hunter Source ⎉](t.me/ghxxx)
ـ●━━━━━━━━━━━━━━●
...
#
#@XR.on(events.CallbackQuery(data="next"))
...
#
#@XR.on(events.CallbackQuery(data="previous"))
